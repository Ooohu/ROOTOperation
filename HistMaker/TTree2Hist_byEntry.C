...
//Load files
	TFile* Prun1 = TFile::Open("../SampleRoot/Reading_Output.root");

	TTree* Tree1 = (TTree*) Prun1->Get("TreeName");
	std::vector< double > flocation;
	std::vector< double >* pflocation = &flocation;
	Tree1->SetBranchAddress("location", &pflocation);

//Prepare histograms
	TH1* hist1 = new TH1F("hist_name", "hist description",10,0,10000);//bins,min,max
